...
async function exportToDocx(project) {
  ...
  const renderDiscussion = (raw) => {
    if (!raw) return [];
    return raw.split("\n").flatMap((line) => {
      const trimmed = line.trim(); if (!trimmed || trimmed.startsWith("[SYSTEM:")) return [];
  ...
  const children = [];
  children.push(new Paragraph({ alignment: AlignmentType.LEFT, spacing: { after: 40 }, children: [new TextRun({ text: "HUMAN-AI INNOVATION COMMONS", font: "Arial", size: 18, bold: true, color: RED, allCaps: true })] }), new Paragraph({ heading: HeadingLevel.HEADING_1, spacing: { after: 80 }, children: [new TextRun({ text: data.patentTitle || name || "Provisional Patent Application", font: "Arial", size: 40, bold: true, color: BLACK })] }), labelValue("Inventor:", data.inventorName), labelValue("Location:", [data.city, data.state, data.country].filter(Boolean).join(", ")), labelValue("Date:", new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })), labelValue("Stage:", SECTIONS[section]?.label || "Complete"), spacer(320));
  children.push(sectionHeading("Our Shared Vision"), spacer(80), bodyText("HAIIC was built on the belief that when AI helps create something valuable, the wealth it generates should flow back to the people AI affects most. Patent Forge is free because democratizing invention is the right thing to do — no fine print, no hidden fees, no claiming ownership of your idea."), spacer(80), bodyText("We're not asking inventors to sign a contract. We're inviting them into a vision. The model we live by distributes the value of AI-assisted innovation equally: one third to the inventor; one third to programs supporting workers displaced by AI; and one third to AI safety research."), spacer(80), bodyText("This is our compass, not a clause. The invention belongs to its inventor. But if it succeeds, we hope they'll remember where the idea started — and consider paying it forward.", { italics: true, color: "888888" }), spacer(80), labelValue("Inventor:", data.inventorName || "—"), labelValue("Date:", new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })), spacer(160));
  if (data.patentTitle || data.patentField || data.summary) { children.push(sectionHeading("Title & Field of Invention"), spacer(80)); if (data.patentTitle) children.push(labelValue("Title:", data.patentTitle)); if (data.patentField) children.push(labelValue("Field:", data.patentField)); if (data.summary) children.push(new Paragraph({ spacing: { after: 40 }, children: [new TextRun({ text: "Summary:", bold: true, font: "Arial", size: 20, color: BLACK })] }), bodyText(data.summary)); children.push(spacer(160)); }
  if (data.noveltyAssessment) children.push(sectionHeading("Novelty & Patentability Assessment"), spacer(80), ...data.noveltyAssessment.split("\n").map(line => new Paragraph({ spacing: { after: line.trim() === "" ? 100 : 60 }, children: [new TextRun({ text: line, font: "Arial", size: 20, color: GRAY })] })), spacer(160));
  ...
  if (data.filingDocument) { children.push(new Paragraph({ children: [new TextRun("")], pageBreakBefore: true }), new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 }, children: [new TextRun({ text: "PROVISIONAL PATENT APPLICATION", font: "Arial", size: 28, bold: true, color: BLACK, allCaps: true })] }), new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 240 }, children: [new TextRun({ text: "Human-AI Innovation Commons", font: "Arial", size: 20, color: GRAY, italics: true })] }), spacer(80), ...data.filingDocument.split("\n").map(line => { const trimmed = line.trim(); const isH = /^[A-Z][A-Z\s]{4,}$/.test(trimmed) && trimmed.length < 60; return new Paragraph({ spacing: { after: trimmed === "" ? 120 : 60 }, ...(isH ? { heading: HeadingLevel.HEADING_2 } : {}), children: [new TextRun({ text: line, font: "Arial", size: isH ? 22 : 20, bold: isH, color: isH ? RED : GRAY })] }); })); }
...
function NoveltyAdvisor({ data, context, onSave }) {
  ...
  const systemPrompt = `You are a knowledgeable friend who has been through the patent process. Give inventors an honest, plain-English read on novelty and patentability — and concrete suggestions to strengthen it. Tone: honest but encouraging.\n\nStructure every response with these exact headers:\n\n🔍 THE HONEST READ\nOne paragraph.\n\n✅ WHAT'S WORKING\n2-3 specific strengths.\n\n⚠️ WATCH OUT FOR\n1-2 prior art concerns in plain English.\n\n💡 HOW TO STRENGTHEN IT\n2-3 actionable suggestions.\n\nEnd with: "Remember: the first idea is rarely the best — every refinement gets you closer. This is a starting point, not a verdict. A registered patent attorney can run a full prior art search before you file."`;
  const runAssessment = async () => { setLoading(true); try { const res = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ system: systemPrompt, messages: [{ role: "user", content: `Assess the novelty and patentability:\n\n${context}` }], max_tokens: 900 }) }); const result = await res.json(); const text = result.content?.map(i => i.type === "text" ? i.text : "").join("\n") || "Unable to generate."; setAssessment(text); const nt = [{ role: "assistant", content: text }]; setThread(nt); onSave({ noveltyAssessment: text, noveltyThread: nt }); } catch { setAssessment("Unable to generate assessment."); } finally { setLoading(false); } };
  const askFollowUp = async () => { if (!followUp.trim() || loading) return; const um = { role: "user", content: followUp }; const nt = [...thread, um]; setThread(nt); setFollowUp(""); setLoading(true); try { const res = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ system: systemPrompt, messages: [{ role: "user", content: `Context:\n\n${context}` }, ...nt], max_tokens: 600 }) }); const result = await res.json(); const text = result.content?.map(i => i.type === "text" ? i.text : "").join("\n") || "Unable to respond."; const updated = [...nt, { role: "assistant", content: text }]; setThread(updated); onSave({ noveltyAssessment: assessment, noveltyThread: updated }); } catch {} finally { setLoading(false); } };
  return (
    <div style={na.wrap}>
      <button onClick={() => setOpen(o => !o)} style={na.toggle}>🔬 Novelty Advisor &nbsp;{open ? "▲" : "▼"}{assessment && <span style={na.badge}>✓ Ready</span>}</button>
      {open && (<div style={na.panel}><p style={na.intro}>An honest read on patentability — and exactly what to do to make it stronger.</p>{!assessment && !loading && <button onClick={runAssessment} style={na.runBtn}>Check Novelty & Patentability →</button>}{loading && <p style={na.loadingMsg}>Analyzing…</p>}{assessment && (<><div style={na.result}><pre style={na.resultText}>{thread[0]?.content || assessment}</pre></div>{thread.length > 1 && <div style={na.threadWrap}>{thread.slice(1).map((m, i) => (<div key={i} style={{ ...na.msg, background: m.role === "user" ? "transparent" : theme.surfaceAlt }}><span style={{ ...na.msgRole, color: m.role === "assistant" ? theme.red : theme.text }}>{m.role === "assistant" ? "Advisor" : "You"}:{"  "}</span><span style={na.msgText}>{m.content}</span></div>))}</div>}<div style={na.followRow}><input style={na.followInput} value={followUp} onChange={e => setFollowUp(e.target.value)} onKeyDown={e => e.key === "Enter" && askFollowUp()} placeholder="Ask a follow-up…" disabled={loading} /><button onClick={askFollowUp} disabled={loading || !followUp.trim()} style={na.askBtn}>Ask →</button></div><button onClick={runAssessment} style={na.rerunBtn}>↻ Re-run</button></>)}</div>)}
    </div>
  );
}
...
function ProjectDashboard({ onNew, onResume, onSignOut, userEmail }) {
  ...
  const handleHandoff = async () => {
    ...
    const project = { id: genId(), user_id: user.id, name: handoff.name || "Brainstorm Import", section: 0, data: { patentTitle: handoff.patentTitle || "", patentField: handoff.patentField || handoff.field || "", summary: handoff.inventionBrief ? handoff.inventionBrief.substring(0, 400) : "", brainstormBrief: handoff.inventionBrief || "", noveltyAssessment: handoff.noveltyAssessment || null, fromBrainstorm: true } };
    await supabase.from(TABLE).insert(project);
    try { localStorage.removeItem(HANDOFF_KEY); } catch {}
    setHandoff(null); onNew(project);
  };
  ...
  const handleNew = async () => {
    ...
    await supabase.from(TABLE).insert(project); setNewName(""); onNew(project);
  };
  ...
  return (
    <div style={ps.content}>
      ...
      {handoff && (
        <div style={hf.banner}>
          <div style={hf.bannerLeft}><div style={hf.bannerTitle}>🔗 Brainstorm session ready to continue</div><div style={hf.bannerMeta}>"{handoff.name}" — title, field, and Invention Brief pre-filled.</div></div>
          <div style={hf.bannerRight}><button onClick={handleHandoff} style={hf.continueBtn}>Continue in Patent Forge →</button><button onClick={() => { try { localStorage.removeItem(HANDOFF_KEY); } catch {} setHandoff(null); }} style={hf.dismissBtn}>Dismiss</button></div>
        </div>
      )}
        ...
        <div style={db.list}>
          ...
          {projects.map(p => (
            <div key={p.id} style={db.card}>
              <div style={db.cardLeft}><div style={db.cardName}>{p.name}{p.data?.fromBrainstorm && <span style={hf.tag}>from Brainstorm</span>}</div><div style={db.cardMeta}>Last saved {new Date(p.updated_at).toLocaleString()} &nbsp;·&nbsp; Stage: <span style={{ color: theme.red }}>{sectionLabel(p.section)}</span></div></div>
              <div style={db.cardRight}><button onClick={() => onResume(p)} style={db.resumeBtn}>Resume →</button><button onClick={() => handleRename(p.id)} style={db.iconBtn} title="Rename">✏</button><button onClick={() => handleDelete(p.id, p.name)} style={db.iconBtn} title="Delete">✕</button></div>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div style={ps.content}>
      ...
      <div style={ps.agreementCard}>
        ...
        <p style={ps.agreementText}>We're not asking you to sign a contract. We're inviting you into a vision. Here's the model we live by — and that we hope inspires you:</p>
        <div style={ps.splits}>
          ...
          <div style={ps.split}><div style={ps.splitPct}>33⅓%</div><div style={ps.splitLabel}>Displaced Workers</div><p style={ps.splitDesc}>AI is reshaping the workforce. Those most affected deserve a share of what it creates.</p></div>
        ...
        <p style={ps.agreementNote}>This is our compass, not a clause. Your invention is yours. But if it succeeds, we hope you'll remember where the idea started, and consider paying it forward.</p>
      </div>
      <label style={ps.checkboxLabel}><input type="checkbox" checked={agreed} onChange={e => { setAgreed(e.target.checked); setData({ ...data, agreed: e.target.checked }); }} style={ps.checkbox} />I've read HAIIC's vision and I'm ready to move forward. I understand this is not a legal obligation — it's an invitation to be part of something better.</label>
...
function TitleSection({ data, setData, onNext }) {
  ...
  return (
    <div style={ps.content}>
      ...
      {data.brainstormBrief && <div style={hf.infoBar}>💡 Your Invention Brief from Brainstorm is saved — the AI will use it as context throughout. Review and refine below if needed.</div>}
...
function DescriptionSection({ data, setData, onNext }) {
  const brainstormCtx = data.brainstormBrief ? `\nInvention Brief from Brainstorm:\n${data.brainstormBrief.substring(0, 1000)}` : "";
  const chat = useChat(`You are a patent drafting assistant at HAIIC helping write the Detailed Description.\nInvention: ${data.patentTitle}\nField: ${data.patentField}\nSummary: ${data.summary}\nInventor: ${data.inventorName}${brainstormCtx}\nAsk about key components, materials, dimensions, alternative embodiments. Push for detail that lets someone reproduce it. After 4-5 exchanges offer to compile a structured description.`);
  const initialized = useRef(false);
  useEffect(() => { if (!initialized.current && chat.messages.length === 0) { initialized.current = true; chat.send(`[SYSTEM: Acknowledge "${data.patentTitle}" and ask the inventor to walk through how it works, starting with main components.]`); } }, []);
  ...
  return (
    <div style={ps.content}>
      ...
      {chat.messages.length > 5 && (<><button onClick={proceed} style={ps.nextBtn}>Next: Draft Claims →</button><NoveltyAdvisor data={data} context={noveltyContext} onSave={(u) => setData({ ...data, ...u })} /></>)}
    </div>
  );
}

function ClaimsSection({ data, setData, onNext }) {
  const chat = useChat(`You are a patent claims drafting assistant at HAIIC.\nInvention: ${data.patentTitle}\nField: ${data.patentField}\nDescription: ${(data.descriptionDiscussion || "").substring(0, 2000)}\nHelp draft patent claims. Start with a broad independent claim, then 2-3 dependent claims. Use proper language: "comprising," "wherein," "configured to". Translate to plain English alongside each claim.`);
  const initialized = useRef(false);
  ...
  return (
    <div style={ps.content}>
      ...
      {chat.messages.length > 4 && (<><button onClick={proceed} style={ps.nextBtn}>Generate Filing Package →</button><NoveltyAdvisor data={data} context={noveltyContext} onSave={(u) => setData({ ...data, ...u })} /></>)}
    </div>
  );
}
...
function ReviewSection({ data, setData }) {
  ...
  const generateDocument = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ system: `Generate a complete provisional patent application for the USPTO with these sections:\n1. HEADER (inventor, location, date)\n2. HAIIC BENEFIT-SHARING ACKNOWLEDGMENT\n3. TITLE OF THE INVENTION\n4. FIELD OF THE INVENTION\n5. BACKGROUND\n6. SUMMARY\n7. DETAILED DESCRIPTION\n8. CLAIMS\n9. ABSTRACT (150 words)\nWrite in formal patent language.`, messages: [{ role: "user", content: `Generate the application:\nInventor: ${data.inventorName}\nLocation: ${data.city}, ${data.state}, ${data.country}\nTitle: ${data.patentTitle}\nField: ${data.patentField}\nSummary: ${data.summary}${bsCtx}\nDescription: ${(data.descriptionDiscussion || "").substring(0, 2500)}\nClaims: ${(data.claimsDiscussion || "").substring(0, 2500)}` }], max_tokens: 4000 }) });
...
export default function PatentForgePage() {
  ...
  useEffect(() => {
    ...
    const save = async () => { await supabase.from(TABLE).update({ section, data, updated_at: new Date().toISOString() }).eq("id", project.id); };
  ...
  const handleDashboard = async () => {
    if (project) await supabase.from(TABLE).update({ section, data, updated_at: new Date().toISOString() }).eq("id", project.id);
